...
Detect::Detect( QLabel* view, QObject *parent ): QObject(parent)
{
    ...
    frame_skip = 0;
    // printf("Demo\n");
    net = parse_network_cfg_custom(cfgfile.toLatin1().data(), 1);
    printf("netcfg complete\n");// set batch=1
    if(!weightfile.isEmpty()){
       load_weights(&net, weightfile.toLatin1().data());
    }
    printf("weightfile complete\n");
...
void Detect::getDetections( IplImage* show_img, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int container_num )
{
        ...
        if (class_id >= 0) {
            ...
            if( class_id == container_num ) {
                ...
                if( show_img->width/2 - 12 < b_x_center && b_x_center < show_img->width/2 + 12 && counted == false ) {
                    product_count++;
                    qDebug() << "Current Product count: " <<  product_count ;
                    counted = true;
                    Group.append(comp);
    ...
    if( !Group.isEmpty() )
    {
        ...
        for( QList<Component>::iterator it = Group.begin(); it != Group.end(); it++ )
        {
            qDebug() << it->toString();
        }
    }
...
void Detect::loadConfig( QString datacfg, QString cfgfile, QString weightfile, float thresh, int cam_index )
{
    char c_name[8] = "classes";
    char n_name[6] = "names";
    char d_name[16] = "data/names.list";
